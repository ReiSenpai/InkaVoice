...
// 🎧 Pestaña 4 — Reproductor de Audioguía con traducción integrada
...
// ─── Textos del capítulo por idioma ─────────────────────────────────────────
const CHAPTER_TEXTS: Record<string, { title: string; desc: string }> = {
  ESPAÑOL: {
    title: 'La Casa de la Gloria del Sol',
    desc:  'Descubre cómo estas enormes piedras, algunas con más de 100 toneladas, fueron talladas con tal precisión que ni una hoja de papel puede deslizarse entre ellas. Su diseño refleja los cuatro suyos del Tahuantinsuyo.',
  },
  ENGLISH: {
    title: "The House of the Sun's Glory",
    desc:  'Discover how these massive stones, some weighing over 100 tons, were carved with such precision that not even a sheet of paper can slide between them. Their design reflects the four suyos of the Tawantinsuyu.',
  },
  QUECHUA: {
    title: 'Inti Wasi Hatun Sumaq',
    desc:  'Kaypi rikusunki imaynatas kay hatun rumikuna, wakin pachaq tunilada nisqamanta astawan, chay chiqap llank\'asqa kasqanta. Mana huk qullqi qullqi chawpipi yaykupuwaqchu.',
  },
};
...
export default function AudioguiaScreen() {
  ...
  const formatTime = (pct: number) => {
    const total = 765; // 12:45 en segundos
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.replace('/resultado')}          >
          <Ionicons name="arrow-back" size={22} color={C.green} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>InkaVoice</Text>
        <TouchableOpacity>
          <Ionicons name="ellipsis-vertical" size={22} color={C.green} />
        </TouchableOpacity>
      </View>

      {/* Región */}
      <Text style={styles.location}>
        {params.region || 'Cusco, Capital Imperial'} · HIGHLANDS REGION
      </Text>
      ...
      <View style={styles.circleContainer}>
        <View style={styles.circleOuter}>
          <View style={styles.imageWrapper}>
            <Image
              source={{ uri: 'https://images.unsplash.com/photo-1587595431973-160d0d94add1?q=80&w=500' }}
              style={styles.coverImage}
            />
          </View>
        </View>
        {/* Arco de progreso decorativo */}
...
const styles = StyleSheet.create({
  ...
  backBtn:            { backgroundColor: C.white, padding: 8, borderRadius: 50, elevation: 3 },
  ...
  arcDecor:           { position: 'absolute', width: 230, height: 230, borderRadius: 115, borderWidth: 4, borderColor: 'transparent', borderTopColor: C.goldL, borderRightColor: C.goldL, top: -10, transform: [{ rotate: '30deg' }] },

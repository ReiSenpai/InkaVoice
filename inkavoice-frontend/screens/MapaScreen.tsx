    ...
    <View style={styles.container}>
      ...
      <View style={styles.mapArea}>
        <View style={styles.fakeMap}>
          <Text style={styles.mapTitle}>Perú</Text>
          {PINS.map(pin => (
            <View key={pin.id} style={[styles.pin, { left: pin.x as `${number}%`, top: pin.y as `${number}%` }]}>
              <View style={[styles.pinCircle, { backgroundColor: pin.color }]}>
                <Ionicons name={pin.icon as any} size={16} color="#FFF" />
              </View>
            </View>
          ))}
        </View>
      ...
      <View style={styles.bottomCard}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {ROUTES.map(route => (
            <TouchableOpacity key={route.id} onPress={() => setSelected(route.id)} style={[styles.card, selected === route.id && styles.cardSelected]}>
        ...
        <TouchableOpacity style={styles.startBtn} onPress={() => navigation.navigate('Routes')}>
...
const styles = StyleSheet.create({
  ...
  mapTitle: { marginTop: 40, textAlign: 'center', fontSize: 30, fontWeight: '800', color: '#B5B5B5' },
  pin: { position: 'absolute' },
  pinCircle: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
  rightButtons: { position: 'absolute', right: 20, top: 30, gap: 14 },
  floatBtn: { width: 50, height: 50, borderRadius: 15, backgroundColor: '#FFF', justifyContent: 'center', alignItems: 'center' },
